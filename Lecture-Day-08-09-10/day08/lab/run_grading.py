...
Config mặc định: retrieval_mode="dense", use_rerank=True (variant tốt nhất)
Để đổi config, truyền argument:
    python run_grading.py --mode hybrid --no-rerank
"""
...
def main():
    ...
    if not questions_path.exists():
        print(f"[ERROR] Không tìm thấy cả {GRADING_QUESTIONS_PATH} lẫn {TEST_QUESTIONS_PATH}")
        sys.exit(1)

    if questions_path == TEST_QUESTIONS_PATH:
        print(f"[WARN] Không tìm thấy grading_questions.json, tạm dùng {TEST_QUESTIONS_PATH.name} để verify pipeline.")
    ...
    print(f"  Input file : {questions_path.name}")
